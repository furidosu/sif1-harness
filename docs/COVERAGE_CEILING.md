...
Six precision passes keep noise low:
...
2. **Cache-listener detection** — for endpoints with no closure-anchor
   match, scan UI files for `Cachable.addListener("$cacheKey", fn)`
   patterns. The registered `fn` receives response_data DIRECTLY (not
   the envelope) so it's harvested in subtree mode with prefix="".
   Decompiler-reused names like `L17_2` (bound to dozens of distinct
   listener bodies across `m_boot/initialize.lua`) are resolved via
   line-aware lookup: pick the definition most-recently defined before
   the binding line.
3. **Single-pass taint with de-taint on reassign** — decompiled
...
4. **For-loop iteration taint** — `for K, V in pairs(tainted)` taints
   the value var; element-index `.[N]` collapses in the merger so
   `cached.unit_list.[1].id` and `cached.unit_list.id` end up at the
   same path.
5. **Helper-following one hop** — when the closure passes a tainted
...
6. **Corpus filter** — a field name appearing on >25% of all m_* UI
...
- **78 endpoints** moved from `ui-only` to `harness-covered` (132 → 54)
- **172 additional unique discovered field paths** surfaced in
  `runtime_listener_observations.json` (81 → 253)
...
done: ~75 endpoints with >=1 kept field, ~35 with 0;
