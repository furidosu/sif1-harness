...
| Endpoints with field-path discoveries | **232** |
| Unique field paths discovered | **949** |
| Endpoints classified `harness-covered` | **266** |
| Endpoints classified `envelope-only` | **31** |
| Endpoints classified `ui-only` | **44** |
...
| Layer | Best tool |
|---|---|
| Top-level + nested response field paths (harness-covered: 266 endpoints) | This harness |
...
| Residual UI-handler endpoints (44 ui-only) where invoke_classes didn't land | Per-endpoint hand-wiring |
